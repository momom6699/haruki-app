#!/usr/bin/env node
/**
 * Overpass の生JSON → LP が読む GeoJSON。
 *
 *   node scripts/build-map-fixture.mjs <overpass.json> [out.geojson]
 *
 * 問い合わせ内容は Android 版 OsmMap.kt の FEATURE_QUERY_TEMPLATE と同じものを使う。
 * `--query` で、そのまま Overpass Turbo に貼れるクエリを表示する。
 */
...
if (args[0] === '--query') {
  ...
  console.log(overpassQuery(lat, lon));
  process.exit(0);
}
if (!args[0]) {
  console.error('usage: build-map-fixture.mjs <overpass.json> [out.geojson]   (or --query [lat] [lon])');
  process.exit(1);
}
const out = args[1] ?? 'public/data/area.geojson';
const geojson = overpassToGeoJSON(JSON.parse(readFileSync(args[0], 'utf8')));
